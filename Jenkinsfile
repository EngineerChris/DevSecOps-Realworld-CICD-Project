...
pipeline{
    ...
    stages {
        ...
        stage('Checkout from Git'){
            steps{
                git branch: 'DevSecOps-Realworld-CICD-Project', url: 'https://github.com/EngineerChris/DevSecOps-Realworld-CICD-Project.git'
            }
        }
            ...
            steps{
                script{
                   withDockerRegistry(credentialsId: 'DockerHub-Credential', toolName: 'Docker'){
                       ...
                       sh "docker tag reddit christianab/reddit:latest "
                       sh "docker push christianab/reddit:latest "
                    }
                }
            }
        }
        stage("Trivy App Image Scan"){
            steps{
                sh "trivy image christianab/reddit:latest > trivy_image_analysis_report.txt"
            }
        }
        ...
        stage('ZAP Dynamic Testing | DAST') {
            steps {
                sshagent(['OWASP-Zap-Credential']) {
                    sh 'ssh -o StrictHostKeyChecking=no ubuntu@18.116.15.52 "docker run -t owasp/zap2docker-stable zap-baseline.py -t http://3.131.99.92:30000/" || true'
    ...
    always {
        ...
        slackSend channel: '#chris-b-devsecops-cicd-alerts', //update and provide your channel name
